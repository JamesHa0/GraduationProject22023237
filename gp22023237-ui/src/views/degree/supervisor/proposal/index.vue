<template>
  <div class="app-container">
    <el-card shadow="never">
      <template #header><span>学生开题报告</span></template>
      <el-table :data="records" v-loading="loading" stripe>
        <el-table-column label="研究背景" min-width="160">
...
import { listProcess, approveProcessSupervisor } from '@/api/degree'
...
import { getSupervisorStudents } from '@/api/degree'
...
async function loadData() {
  ...
  try {
    ...
    const res = await listProcess({ thesisId: thesisIds.join(','), processType: 3, pageNum: pageNum.value, pageSize: pageSize.value })
